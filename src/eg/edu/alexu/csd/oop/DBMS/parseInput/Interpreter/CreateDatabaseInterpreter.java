...
public class CreateDatabaseInterpreter implements Interpreter {
    private boolean valid(String query){
        query=query.trim();
        String createRegex = "CREATE+\\s+DATABASE+\\s+([a-zA-Z]:)?(\\\\?[a-zA-Z0-9_.-]+)+\\\\? *;*";
        Pattern pattern  = Pattern.compile(createRegex, Pattern.CASE_INSENSITIVE);
